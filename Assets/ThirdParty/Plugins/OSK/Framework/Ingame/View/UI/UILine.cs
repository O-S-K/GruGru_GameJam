...
namespace OSK
{
    public class UILine : Graphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            if (LinePoints.Count > 1)
            {
                ...
                for (int i = 1; i < LinePoints.Count; i++)
                {
                    ...
                    Vector2 lineDirPerp = MathUtils.Rotate(lineDir, -90).normalized;
        ...
        private void PopulateCircle(VertexHelper vh, Vector2 center, Vector2 direction, float angle)
        {
            ...
            Vector2 startAngleDir = MathUtils.Rotate(direction, angle / 2f).normalized;
            Vector2 prevAngleDir = startAngleDir;

            for (int i = 1; i <= lineRoundness; i++)
            {
                Vector2 angleDir = MathUtils.Rotate(startAngleDir, -i * angleStep).normalized;
